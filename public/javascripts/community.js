...
async function community_getTablesData() {
    ...
    for (let i = 0; i < response.length; i++) {
        const createDay = response[i].created_at.split('T');
        console.log(createDay);
        createDay[1] = createDay[1].slice(0, createDay[1].length - 5);
        response[i].created_at = createDay[0] + ' ' + createDay[1];
    }

    console.log(response);
    boardObj.tableDatas = response;
...
function community_onClickBoardTitle(e) {
    ...
    const index = boardObj.table.row(this)[0];
    const data = boardObj.tableDatas[index];
