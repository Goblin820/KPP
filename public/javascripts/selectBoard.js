...
function selectBoard_initObj() {
    ...
    selectBoardObj.titleElements.span = titleElements[0];
    selectBoardObj.titleElements.input = titleElements[1];

    selectBoardObj.titleElements.input.hidden = true;

    selectBoardObj.textElements.standard = document.getElementById('text');
}
...
async function selectBoard_initDataSet() {
    ...
    selectBoardObj.titleElements.span.innerText = boardResponse.title;
    selectBoardObj.textElements.standard.innerHTML = boardResponse.text;
...
function selectBoard_onClickModifyOkBtn(e) {
    selectBoardObj.data.title = selectBoardObj.titleElements.input.value;
    selectBoardObj.data.text = selectBoardObj.textElements.$summerNote.text();

    $.ajax({
        url: '/board',
        method: 'patch',
        dataType: 'json',
        data: JSON.stringify(selectBoardObj.data),
        success: function (res) {
            if (res.result == 'success') {
                Swal.fire({
                    icon: 'success',
                    ...
                    window.location.replace(selectBoardObj.data.returnTo);
                });
            }
        },
    });
...
function selectBoard_setModifyMode(isMode) {
    if (isMode) {
        selectBoardObj.titleElements.span.hidden = isMode;
        selectBoardObj.titleElements.input.hidden = !isMode;
        selectBoardObj.titleElements.input.value = selectBoardObj.data.title;

        selectBoardObj.textElements.standard.hidden = isMode;
